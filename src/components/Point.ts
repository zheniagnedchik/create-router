...
class Point {
  ...
  public transform(
    ...
    const x = new Point(
			scaleX *
				((this.lat - centerX) * Math.cos(degInRad) -
					(this.lng - centerY) * Math.sin(degInRad)) +
				centerX,
			scaleY *
				((this.lat - centerX) * Math.sin(degInRad) +
					(this.lng - centerY) * Math.cos(degInRad)) +
				centerY
		);
		return x;
  ...
  public equal(p: Point) {
    ...
    return (
      Math.abs(p.getLatLng().lat - this.lat) < EPSILON &&
      Math.abs(p.getLatLng().lng - this.lng) < EPSILON
    );
  }
}
