...
export default class FlightRoutePlanner {
  public static planForConvexPolygon(
    polygon: Polygon,
    space: number,
    rotate: number
  ) {
    ...
    for (var i = 0; i < latLines.len; i++) {
      ...
      for (var j = 0; j < rotatedPolygonVertices.length; j++) {
        ...
        const vertex2 =
          rotatedPolygonVertices[(j + 1) % rotatedPolygonVertices.length];
        const vector = new Vector(
          vertex1.getPointInArray(),
          vertex2.getPointInArray()
        );
        const point = vector.getPointOnVectorWithY(
          rotatedPolygon.getOutterBound().vertices[0].getLatLng().lat -
            i * latLines.lat
        );
      ...
      if (
        line.length < 2 ||
        (line[0].getLatLng().lat === line[1].getLatLng().lat &&
          line[0].getLatLng().lng === line[1].getLatLng().lng)
      ) {
        continue;
      }
  ...
  public static planForConcavePolygon(
    polygon: Polygon,
    space: number,
    rotate: number
  ) {
    ...
    for (let i = 0; i < convexPolygons.length; i++) {
      ...
      polylines = polylines.concat(
        this.planForConvexPolygon(convexPolygon, space, rotate)
      );
    }

    return polylines;
  }
  public static planForConcavePolygon2(
    polygon: Polygon,
    space: number,
    rotate: number,
    findIntersectionOnly?: boolean
  ) {
    const result = [];
      ...
      lines.push(
        line.sort((a, b) => {
          return a.getLatLng().lng === b.getLatLng().lng
            ? 0
            : a.getLatLng().lng < b.getLatLng().lng
            ? -1
            : 1;
        })
      );
    ...
    if (!findIntersectionOnly) {
      ...
      while (lines.length > 0) {
        ...
        const firstTwoValidPoints = this.getFirstPairOfPoint(
          rotatedPolygon,
          polylines,
          line,
          direction
      ...
      // draw lines
      const polylinesPolygon = new Polygon(polylines);
      const rotatedPolylines = polylinesPolygon.rotate(
        rotate,
        polygon.getOutterBound().center
      );
      const rotatedPolylinesVertices = rotatedPolylines.getVertices();
      const recordedLines = [];
      ...
      // Below are just drawing the polylines on Gaode map
      for (let i = 0; i < rotatedPolylinesVertices.length - 1; i++) {
        // const l = GaodeHelper.getInstance().drawPolyline(
        // 	rotatedPolylinesVertices[i],
        // 	rotatedPolylinesVertices[i + 1],
        // 	randomHexColorCode(),
        // 	12
        // );

        recordedLines.push(rotatedPolylinesVertices);
      }

      return recordedLines[0];
    }

    return [];
  }

  private static getFirstPairOfPoint(
    polygon: Polygon,
